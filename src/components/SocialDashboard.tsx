...
import { Bell, ChevronLeft, Coins, Crown, Gift, Heart, Home, MessageCircle, Mic, Music2, Pause, Play, Radio, Search, Send, Share2, UserRound, Volume2, Wallet, X, Youtube } from 'lucide-react';
...
export interface SocialProfile extends LocalProfile {
  displayName: string;
  likes: number;
  views: number;
}
...
export default function SocialDashboard({ session, onSignOut }: SocialDashboardProps) {
  ...
  const renderActiveView = () => {
    switch (activeTab) {
      ...
      case 'moments':
        return <MomentsFeedView onShare={(message, image) => addMoment(message, image)} profile={profile} />;
      case 'matching':
      ...
      case 'mine':
        return <MineView profile={profile} wallet={wallet} onRecharge={() => setRechargeOpen(true)} onProfile={() => profile && selectProfile(profile)} onSignOut={onSignOut} />;
      case 'room':
        return <HomeView people={people} onRoom={openParty} onProfile={selectProfile} onInvite={() => setInviteOpen(true)} />;
      default:
        return <HomeView people={people} onRoom={openParty} onProfile={selectProfile} onInvite={() => setInviteOpen(true)} />;
...
function MomentsView({ onShare, profile }: { onShare: (message: string, image?: string) => void; profile: SocialProfile | null }) {
  ...
  const handleShare = async () => {
    ...
    const payload = { user_id: profile?.id || 'local-user', author_name: profile?.displayName || 'You', content: text.trim() || 'Shared a moment.', image_url: image.trim() || mediaPreview || null };
    try {
...
function InviteBottomSheet({ open, onClose, people }: InviteBottomSheetProps) { if (!open) return null; return <div className="fixed inset-0 z-[70] flex items-end bg-black/60"><motion.section initial={{ y: '100%' }} animate={{ y: 0 }} className="w-full rounded-t-[2rem] bg-[#211a32] p-6"><div className="mx-auto max-w-2xl"><div className="flex items-center justify-between"><h2 className="text-2xl font-black">Invite friends</h2><button onClick={onClose}><X /></button></div><div className="mt-5 grid grid-cols-4 gap-3 text-center text-xs font-bold"><span>🟢<br />WhatsApp</span><span>🔵<br />Facebook</span><span>🌈<br />Instagram</span><span>✨<br />Moment</span></div><div className="mt-6 space-y-2">{people.slice(0, 4).map((person) => <div key={person.id} className="flex items-center gap-3 rounded-xl bg-white/5 p-3"><span className="flex-1 text-sm font-bold">{person.displayName}</span><button className="rounded-lg bg-rose-500 px-3 py-2 text-xs font-black">Share</button></div>)}</div></div></motion.section></div>; }

function ChatTabView({ people, onProfile, onRoom }: { people: SocialProfile[]; onProfile: (person: SocialProfile) => void; onRoom: () => void }) {
  const matches = people.length ? people.slice(0, 8) : Array.from({ length: 4 }, (_, index) => ({ id: `mock-match-${index}`, age: 24 + index, gender: '', city: 'Nearby', bio: 'Ready to connect in the live room.', interests: ['Music'], avatarUrl: '', latitude: null, longitude: null, matchDistance: 25, displayName: ['Iris', 'Aarav', 'Maya', 'Noah'][index], likes: 12 + index, views: 80 + index * 10 }));
  const [selected, setSelected] = useState<SocialProfile>(matches[0]);
  const [message, setMessage] = useState('');
  return <section className="space-y-4"><div className="flex items-end justify-between"><div><p className="text-xs font-black uppercase tracking-[0.2em] text-rose-300">Matches</p><h1 className="mt-2 text-2xl font-black">Chat & voice party</h1></div><button type="button" onClick={onRoom} className="flex items-center gap-2 rounded-xl bg-rose-500 px-3 py-2 text-xs font-black"><Radio className="h-4 w-4" />Voice Party</button></div><div className="grid gap-3 lg:grid-cols-[.65fr_1.35fr]"><div className="rounded-2xl border border-white/10 bg-white/5 p-2"><p className="px-2 pb-2 text-[10px] font-black uppercase tracking-[0.2em] text-white/40">Your matches</p>{matches.map((person) => <button type="button" key={person.id} onClick={() => setSelected(person)} className={`flex w-full items-center gap-2 rounded-xl p-2 text-left ${selected.id === person.id ? 'bg-rose-500/15' : 'hover:bg-white/10'}`}><div className="flex h-9 w-9 items-center justify-center overflow-hidden rounded-full bg-gradient-to-br from-violet-600 to-rose-500">{person.avatarUrl ? <img src={person.avatarUrl} alt={person.displayName} className="h-full w-full object-cover" /> : <UserRound className="h-4 w-4" />}</div><span className="min-w-0 flex-1 truncate text-xs font-black">{person.displayName}</span><MessageCircle className="h-3.5 w-3.5 text-white/30" /></button>)}</div><div className="rounded-2xl border border-white/10 bg-white/5 p-3"><button type="button" onClick={() => onProfile(selected)} className="flex items-center gap-2 text-left"><div className="flex h-10 w-10 items-center justify-center overflow-hidden rounded-full bg-gradient-to-br from-violet-600 to-rose-500">{selected.avatarUrl ? <img src={selected.avatarUrl} alt={selected.displayName} className="h-full w-full object-cover" /> : <UserRound className="h-4 w-4" />}</div><span><strong className="block text-sm">{selected.displayName}</strong><small className="text-xs text-emerald-300">Active now</small></span></button><div className="mt-4 flex min-h-44 flex-col justify-end gap-2 rounded-xl bg-black/15 p-3"><p className="max-w-[80%] rounded-xl bg-white/10 p-2 text-xs">Hey, want to join the live room?</p><p className="max-w-[80%] self-end rounded-xl bg-rose-500/80 p-2 text-xs">I am in. Send me the invite.</p>{message && <p className="max-w-[80%] self-end rounded-xl bg-rose-500/80 p-2 text-xs">{message}</p>}</div><form onSubmit={(event) => { event.preventDefault(); if (message.trim()) setMessage(''); }} className="mt-2 flex gap-2"><input value={message} onChange={(event) => setMessage(event.target.value)} placeholder="Write a message..." className="min-w-0 flex-1 rounded-xl border border-white/10 bg-black/20 px-3 py-2.5 text-xs outline-none" /><button type="submit" aria-label="Send message" className="rounded-xl bg-rose-500 p-2.5"><Send className="h-4 w-4" /></button></form></div></div></section>;
}

function MomentsFeedView({ onShare, profile }: { onShare: (message: string, image?: string) => void; profile: SocialProfile | null }) {
  const [items, setItems] = useState<Array<{ id: string; author: string; content: string; image?: string }>>(() => profile ? [{ id: 'welcome-moment', author: profile.displayName, content: profile.bio || 'Ready to meet new people in the room.', image: profile.avatarUrl }] : []);
  const [text, setText] = useState('');
  const [image, setImage] = useState('');
  const publish = () => { if (!text.trim() && !image) return; const item = { id: crypto.randomUUID(), author: profile?.displayName || 'You', content: text.trim() || 'Shared a new moment.', image: image || profile?.avatarUrl }; setItems((current) => [item, ...current]); onShare(item.content, item.image); setText(''); setImage(''); };
  return <section className="space-y-4"><div><p className="text-xs font-black uppercase tracking-[0.2em] text-rose-300">Moments</p><h1 className="mt-2 text-2xl font-black">Main feed</h1></div><div className="rounded-2xl border border-white/10 bg-white/5 p-3"><textarea value={text} onChange={(event) => setText(event.target.value)} rows={2} placeholder="Share what is happening..." className="w-full resize-none rounded-xl border border-white/10 bg-black/20 p-2.5 text-xs outline-none" /><div className="mt-2 flex gap-2"><input value={image} onChange={(event) => setImage(event.target.value)} placeholder="Paste an image URL" className="min-w-0 flex-1 rounded-xl border border-white/10 bg-black/20 px-2.5 py-2 text-xs outline-none" /><button type="button" onClick={publish} className="rounded-xl bg-rose-500 px-3 py-2 text-xs font-black">Post</button></div></div><div className="grid gap-4 md:grid-cols-2">{items.map((item) => <article key={item.id} className="overflow-hidden rounded-2xl border border-white/10 bg-white/5"><div className="flex items-center gap-2 p-3"><div className="flex h-9 w-9 items-center justify-center overflow-hidden rounded-full bg-gradient-to-br from-violet-600 to-rose-500">{item.image ? <img src={item.image} alt={item.author} className="h-full w-full object-cover" /> : <UserRound className="h-4 w-4" />}</div><p className="text-xs font-black">{item.author}</p></div>{item.image ? <img src={item.image} alt="Moment" className="h-64 w-full object-cover" /> : <div className="flex h-64 items-center justify-center bg-gradient-to-br from-violet-700/50 via-fuchsia-600/30 to-rose-600/40 text-4xl">✨</div>}<p className="p-3 text-sm text-white/75">{item.content}</p></article>)}</div></section>;
}
...
function WarningModal({ mode, onCancel, onConfirm }: { mode: RoomMode; onCancel: () => void; onConfirm: () => void }) { return <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/75 p-4"><motion.div initial={{ scale: .92, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="w-full max-w-md rounded-3xl bg-[#241b36] p-6 shadow-2xl ring-1 ring-red-300/30"><div className="flex items-center justify-between"><h2 className="text-xl font-black text-red-200">Start {mode} room?</h2><button type="button" onClick={onCancel}><X /></button></div><p className="mt-4 text-sm leading-6 text-white/75">Live rooms must remain respectful and safe. Nudity, pornography, violence, and adult content are prohibited.</p><div className="mt-6 flex gap-3"><button type="button" onClick={onCancel} className="flex-1 rounded-xl border border-white/15 py-3 text-sm font-bold text-white/70">Cancel</button><button type="button" onClick={onConfirm} className="flex-1 rounded-xl bg-red-500 py-3 text-sm font-black text-white">I Agree & Start</button></div></motion.div></div>; }
