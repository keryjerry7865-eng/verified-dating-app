...
import { Bell, Heart, LogOut, MessageCircle, Send, UserRound, X } from 'lucide-react';
import type { Session } from '@supabase/supabase-js';
import { supabase } from '../supabaseClient';

type Profile = { id: string; display_name: string; age: number; gender: string; city: string; interests: string[]; avatar_url: string | null };
...
export default function Dashboard({ session, onSignOut }: DashboardProps) {
  ...
  const [activeTab, setActiveTab] = useState<'swipe' | 'likes' | 'chat'>('swipe');
  ...
  useEffect(() => {
    const loadDashboard = async () => {
      ...
      setDiscoverProfiles((others || []) as Profile[]);
      ...
      if (matchIds.length) {
        ...
        setLikedProfiles((matchProfiles || []) as Profile[]);
      }
      ...
      if (incomingIds.length) {
        ...
        setIncomingLikes((incomingProfiles || []) as Profile[]);
      }
    };
    loadDashboard();
  }, [session.user.id]);
  ...
  const likeProfile = async (target: Profile) => {
    ...
    if (isMutual) {
      ...
      pushAlert(`You matched with ${target.display_name}!`);
    }
  };
...
function ProfileTile({ profile, onChat }: { profile: Profile; onChat: () => void }) { return <div className="flex items-center gap-3 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-gray-100"><div className="h-14 w-14 rounded-full bg-rose-100" /> <div className="min-w-0 flex-1"><p className="truncate font-bold">{profile.display_name}</p><p className="text-xs text-gray-500">{profile.city}</p></div><button onClick={onChat} className="rounded-xl bg-rose-500 px-3 py-2 text-xs font-bold text-white">Chat</button></div>; }
