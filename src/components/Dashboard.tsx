...
type Profile = { id: string; age: number; gender: string; city: string; interests: string[]; avatar_url: string | null; displayName?: string };
  ...
  useEffect(() => {
    const loadDashboard = async () => {
      const [{ data: current }, { data: others }] = await Promise.all([
        ...
        supabase.from('profiles').select('id,age,gender,city,interests,avatar_url').neq('id', session.user.id).limit(20),
      ]);
      ...
      if (matchIds.length) {
        const { data: matchProfiles } = await supabase.from('profiles').select('id,age,gender,city,interests,avatar_url').in('id', matchIds);
      ...
      if (incomingIds.length) {
        const { data: incomingProfiles } = await supabase.from('profiles').select('id,age,gender,city,interests,avatar_url').in('id', incomingIds);
