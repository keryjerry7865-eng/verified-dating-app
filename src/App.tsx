...
function ProfileSettingsManager({ profile, setProfile, incomeWallet, setIncomeWallet, upiId, setUpiId }: { profile: Profile; setProfile: Setter<Profile>; incomeWallet: number; setIncomeWallet: Setter<number>; upiId: string; setUpiId: (value: string) => void }) { const [status, setStatus] = useState(''); const withdraw = () => { if (!upiId.trim()) return setStatus('Enter a UPI ID first.'); if (!incomeWallet) return setStatus('Income wallet is empty.'); setIncomeWallet(0); setStatus('Withdrawal request submitted.'); }; return <section className="space-y-3 p-3"><div className="flex items-center gap-3 rounded-2xl bg-gradient-to-br from-pink-700 to-indigo-950 p-4"><div className="relative"><img src={profile.avatar} alt={profile.name} className="h-16 w-16 rounded-full object-cover" /><label className="absolute bottom-0 right-0 cursor-pointer rounded-full bg-pink-600 p-1.5"><Camera className="h-3.5 w-3.5" /><input type="file" accept="image/*" onChange={(event) => { const file=event.target.files?.[0]; if (!file) return; const reader=new FileReader(); reader.onload=() => setProfile((current) => ({ ...current, avatar: String(reader.result) })); reader.readAsDataURL(file); }} className="hidden" /></label></div><div><h2 className="text-lg font-black">{profile.name}</h2><p className="text-xs text-white/70">{profile.city} · Noble Level 3</p></div></div><div className="space-y-2 rounded-2xl border border-slate-800 bg-slate-950 p-3"><p className="text-xs font-black">Profile configuration</p><label className="block text-[10px] font-bold text-slate-400">User Age Tracker<input type="number" min="18" max="100" value={profile.age} onChange={(event) => setProfile((current) => ({ ...current, age: Number(event.target.value) }))} className="mt-1 w-full rounded-xl bg-slate-900 p-2 text-xs" /></label><label className="block text-[10px] font-bold text-slate-400">Target State Location<select value={profile.state} onChange={(event) => setProfile((current) => ({ ...current, state: event.target.value }))} className="mt-1 w-full rounded-xl bg-slate-900 p-2 text-xs"><option>West Bengal</option><option>Maharashtra</option><option>Delhi</option><option>Karnataka</option><option>Tamil Nadu</option><option>Telangana</option><option>Uttar Pradesh</option><option>Rajasthan</option><option>Gujarat</option><option>Kerala</option></select></label><label className="block text-[10px] font-bold text-slate-400">University / College<input value={profile.college} onChange={(event) => setProfile((current) => ({ ...current, college: event.target.value }))} className="mt-1 w-full rounded-xl bg-slate-900 p-2 text-xs" /></label><label className="block text-[10px] font-bold text-slate-400">Qualification<select value={profile.qualification} onChange={(event) => setProfile((current) => ({ ...current, qualification: event.target.value }))} className="mt-1 w-full rounded-xl bg-slate-900 p-2 text-xs"><option>B.Tech</option><option>M.A</option><option>MBA</option><option>Graduate</option></select></label><label className="block text-[10px] font-bold text-slate-400">Job Profile<input value={profile.jobProfile} onChange={(event) => setProfile((current) => ({ ...current, jobProfile: event.target.value }))} className="mt-1 w-full rounded-xl bg-slate-900 p-2 text-xs" /></label><label className="block text-[10px] font-bold text-slate-400">Max Match Distance: {profile.distancePreference} km<input type="range" min="5" max="100" value={profile.distancePreference} onChange={(event) => setProfile((current) => ({ ...current, distancePreference: Number(event.target.value) }))} className="mt-1 w-full accent-pink-500" /></label><input value={profile.name} onChange={(event) => setProfile((value) => ({ ...value, name: event.target.value }))} className="w-full rounded-xl bg-slate-900 p-2 text-xs" placeholder="Display name" /><input value={profile.city} onChange={(event) => setProfile((value) => ({ ...value, city: event.target.value }))} className="w-full rounded-xl bg-slate-900 p-2 text-xs" placeholder="City" /><textarea value={profile.bio} onChange={(event) => setProfile((value) => ({ ...value, bio: event.target.value }))} className="w-full rounded-xl bg-slate-900 p-2 text-xs" placeholder="Bio" /></div><div className="space-y-2 rounded-2xl border border-emerald-500/20 bg-slate-950 p-3"><p className="text-xs font-black text-emerald-300">Income Wallet · ₹{incomeWallet}</p><div className="flex gap-2"><input value={upiId} onChange={(event) => setUpiId(event.target.value)} placeholder="yourname@upi" className="min-w-0 flex-1 rounded-xl bg-slate-900 p-2 text-xs" /><button type="button" onClick={withdraw} className="rounded-xl bg-emerald-600 px-3 text-[10px] font-black">Withdraw</button></div>{status && <p className="text-[10px] text-emerald-300">{status}</p>}</div></section>; }
